...
{ 
    ...
    std::string execPlusArgs(executable.get());
    execPlusArgs += " ";
    execPlusArgs += cmdline.get();

    if (!::CreateProcessA(NULL, const_cast<char *>(execPlusArgs.c_str()), NULL, NULL, TRUE, 0, envstrs.get(), workdir.get(), si.get(), &pi))
